...
export function VideoPlayer({ video, categoryName }: VideoPlayerProps) {
  ...
  return (
    <div>
      <div className="mb-2 sm:mb-4">
        <h2 className="text-base sm:text-lg md:text-xl font-medium line-clamp-2">{video.title}</h2>
        {categoryName && (
      ...
      <div 
        className={`relative bg-black rounded-lg overflow-hidden video-container ${isSimulatedFullscreen ? 'simulated-fullscreen' : ''}`}
        ...
        style={{
          ...
          ...(typeof document !== 'undefined' && typeof window !== 'undefined' ? 
              (document?.fullscreenElement && window?.innerWidth < 768 ? {
                transform: 'rotate(90deg)',
                width: '90vh',  /* Reduced from 100vh to 90vh to fix overflow */
                height: '80vw', /* Reduced from 100vw to 80vw to fix overflow */
                position: 'fixed',
                top: '10%',    /* Add top offset to center the video */
                left: '5%',    /* Add left offset to center the video */
                maxWidth: '90vh', /* Ensure video doesn't exceed screen size */
                maxHeight: '80vw', /* Ensure video doesn't exceed screen size */
                overflow: 'hidden',
              } : {}) 
            : {})
        }}
      >
        {isYouTubeVideo ? (
          <YouTubePlayer 
            ...
            onEnded={() => { setIsPlaying(false); setVideoProgress(0); }}
